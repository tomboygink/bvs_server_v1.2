...
export class ThermalWell {
  ...
  async insertThermalWell() {
    var db_response = await this.db.query(
      "INSERT INTO skvazhiny(number, org_id, group_id, dev_id, create_at) " +
        "VALUES('" +
        this.args.number +
        "'," +
        this.args.org_id +
  ...
  // Получение всех термоскважин
  async selectAllThermalWell(): Promise<ThermalWellEntity[]> {
    const db_response = await this.db.query("SELECT * FROM skvazhiny");
    let result: ThermalWellEntity[] = new Array();
  ...
  async selectThermalWell(): Promise<ThermalWellEntity[]> {
    var db_response = await this.db.query(
      "SELECT * FROM skvazhiny WHERE group_id = " + this.args.group_dev_id
    );
