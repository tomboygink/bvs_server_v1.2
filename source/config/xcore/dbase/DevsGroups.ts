...
export class DevsGroups {
    ...
    async insertDevsGroup() {
        var db_response = await this.db.query("INSERT INTO devs_groups(parent_id, g_name, latitude, longitude, org_id, ord_num, deleted, g_info)" +
            "VALUES(" + this.args.parent_id + ", \'" + this.args.g_name + "\', \'" + this.args.latitude + "\', \'" + this.args.longitude +
            "\', " + this.args.id_org + ", 0, " + this.args.deleted + ", \'" + this.args.g_info + "\') RETURNING id");
    ...
    async updateDevsGroup() {
        try {
            ...
            await this.db.query("UPDATE devs_groups SET parent_id =" + this.args.parent_id + ", g_name = \'" + this.args.g_name + "\', latitude = \'" + this.args.latitude +
                "\', longitude = \'" + this.args.longitude + "\' org_id = " + this.args.org_id + ", deleted = " + this.args.deleted +
                ", g_info = " + this.args.g_info + " WHERE id = " + this.args.id);
