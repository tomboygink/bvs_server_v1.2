...
export class Devs {
    ...
    async insertDevs() {
        var db_response = await this.db.query("INSERT INTO devs(group_dev_id, number, name, latitude, longitude, sensors, deleted, info, period_sess) " +
    ...
    async updateDevs() {
        var db_response = await this.db.query("UPDATE devs SET group_dev_id = " + this.args.group_dev_id + ", number = \'" + this.args.number + "\', name = \'" + this.args.name + "\', latitude = \'" + this.args.latitude + "\', " +
